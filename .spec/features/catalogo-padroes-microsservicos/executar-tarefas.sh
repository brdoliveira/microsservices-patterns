#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano catalogo-padroes-microsservicos` em 2026-09-02 00:20
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo catalogo-padroes-microsservicos
set -u
set -o pipefail

RUN_ID='kuala-lumpur-catalogo-padroes-microsservicos-mtjcnsgu'
FEATURE='catalogo-padroes-microsservicos'
BASE_BRANCH='spec/catalogo-padroes-microsservicos'
ENGINE='/Users/brufe/.agents/skills/onp-spec-driven/scripts/onp-spec.mjs'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/catalogo-padroes-microsservicos/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/kuala-lumpur-catalogo-padroes-microsservicos-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/kuala-lumpur-catalogo-padroes-microsservicos"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/catalogo-padroes-microsservicos/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/catalogo-padroes-microsservicos-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-001' 'Você executa UMA tarefa da feature "catalogo-padroes-microsservicos" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-padroes-microsservicos/spec.md, .spec/features/catalogo-padroes-microsservicos/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-001 — "Configurar base Java e verificacoes do catalogo"
  critérios/refs: AC-006 (Codigo publico possui JavaDoc), AC-008 (Build e testes validam exemplos Java), AC-009 (Verificacao automatica protege a estrutura de documentacao)
  arquivos permitidos (e seus testes): pom.xml, catalog-validation/src/test/java, catalog-validation/pom.xml
  mensagem de commit: "T-001 catalogo-padroes-microsservicos: Configurar base Java e verificacoes do catalogo"

Regras inegociáveis:
- Todo critério de aceite referenciado vira teste com @spec:AC-xxx no título.
- NUNCA enfraqueça, pule (skip/todo) ou apague um teste para passar — teste pulado não é prova e o audit acusa.
- Rode os testes localmente com `mvn test` até passarem.
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/catalogo-padroes-microsservicos-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/catalogo-padroes-microsservicos-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-002' 'Você executa UMA tarefa da feature "catalogo-padroes-microsservicos" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-padroes-microsservicos/spec.md, .spec/features/catalogo-padroes-microsservicos/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-002 — "Escrever README raiz com indice dos padroes"
  critérios/refs: AC-001 (README raiz apresenta os padroes disponiveis), AC-002 (README raiz redireciona para cada pasta de padrao)
  arquivos permitidos (e seus testes): README.md
  mensagem de commit: "T-002 catalogo-padroes-microsservicos: Escrever README raiz com indice dos padroes"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/catalogo-padroes-microsservicos-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/catalogo-padroes-microsservicos-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-003' 'Você executa UMA tarefa da feature "catalogo-padroes-microsservicos" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-padroes-microsservicos/spec.md, .spec/features/catalogo-padroes-microsservicos/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-003 — "Documentar e exemplificar padroes de entrada e descoberta"
  critérios/refs: AC-003 (Cada pasta de padrao tem README explicativo), AC-004 (README de padrao conecta o conteudo ao restante do catalogo), AC-005 (Cada padrao possui codigo Java no proprio diretorio), AC-006 (Codigo publico possui JavaDoc), AC-007 (O repositorio usa Java para os exemplos de implementacao)
  arquivos permitidos (e seus testes): api-gateway/README.md, api-gateway/pom.xml, api-gateway/src/main/java, service-discovery/README.md, service-discovery/pom.xml, service-discovery/src/main/java
  mensagem de commit: "T-003 catalogo-padroes-microsservicos: Documentar e exemplificar padroes de entrada e descoberta"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/catalogo-padroes-microsservicos-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/catalogo-padroes-microsservicos-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-004' 'Você executa UMA tarefa da feature "catalogo-padroes-microsservicos" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-padroes-microsservicos/spec.md, .spec/features/catalogo-padroes-microsservicos/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-004 — "Documentar e exemplificar padroes de resiliencia"
  critérios/refs: AC-003 (Cada pasta de padrao tem README explicativo), AC-004 (README de padrao conecta o conteudo ao restante do catalogo), AC-005 (Cada padrao possui codigo Java no proprio diretorio), AC-006 (Codigo publico possui JavaDoc), AC-007 (O repositorio usa Java para os exemplos de implementacao)
  arquivos permitidos (e seus testes): circuit-breaker/README.md, circuit-breaker/pom.xml, circuit-breaker/src/main/java, bulkhead/README.md, bulkhead/pom.xml, bulkhead/src/main/java
  mensagem de commit: "T-004 catalogo-padroes-microsservicos: Documentar e exemplificar padroes de resiliencia"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/catalogo-padroes-microsservicos-faixa-4' "$WT" "$st" || return 1
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/catalogo-padroes-microsservicos-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-005' 'Você executa UMA tarefa da feature "catalogo-padroes-microsservicos" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-padroes-microsservicos/spec.md, .spec/features/catalogo-padroes-microsservicos/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-005 — "Documentar e exemplificar padroes de dados e transacao"
  critérios/refs: AC-003 (Cada pasta de padrao tem README explicativo), AC-004 (README de padrao conecta o conteudo ao restante do catalogo), AC-005 (Cada padrao possui codigo Java no proprio diretorio), AC-006 (Codigo publico possui JavaDoc), AC-007 (O repositorio usa Java para os exemplos de implementacao)
  arquivos permitidos (e seus testes): database-per-service/README.md, database-per-service/pom.xml, database-per-service/src/main/java, saga/README.md, saga/pom.xml, saga/src/main/java, cqrs/README.md, cqrs/pom.xml, cqrs/src/main/java
  mensagem de commit: "T-005 catalogo-padroes-microsservicos: Documentar e exemplificar padroes de dados e transacao"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/catalogo-padroes-microsservicos-faixa-5' "$WT" "$st" || return 1
...
executar_faixa_6() {
  ...
  preparar_worktree 'faixa-6' 'spec/catalogo-padroes-microsservicos-faixa-6' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-6' --estado executando --tentativa "$(tentativa 'faixa-6')"
  : > "$LOG_DIR/faixa-6.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-6' 'T-006' 'Você executa UMA tarefa da feature "catalogo-padroes-microsservicos" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/catalogo-padroes-microsservicos/spec.md, .spec/features/catalogo-padroes-microsservicos/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-006 — "Documentar e exemplificar padroes de integracao e evolucao"
  critérios/refs: AC-003 (Cada pasta de padrao tem README explicativo), AC-004 (README de padrao conecta o conteudo ao restante do catalogo), AC-005 (Cada padrao possui codigo Java no proprio diretorio), AC-006 (Codigo publico possui JavaDoc), AC-007 (O repositorio usa Java para os exemplos de implementacao)
  arquivos permitidos (e seus testes): event-driven-architecture/README.md, event-driven-architecture/pom.xml, event-driven-architecture/src/main/java, strangler-fig/README.md, strangler-fig/pom.xml, strangler-fig/src/main/java, externalized-configuration/README.md, externalized-configuration/pom.xml, externalized-configuration/src/main/java
  mensagem de commit: "T-006 catalogo-padroes-microsservicos: Documentar e exemplificar padroes de integracao e evolucao"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-6' 'spec/catalogo-padroes-microsservicos-faixa-6' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/catalogo-padroes-microsservicos/executar-tarefas.sh --gate"
      exit 0
    fi
